...
function resetScene(){
    for (var i = 0; i < meshOld.length; i++) {
        var vertexList = VMeshToVList(meshOld[i]);
        var normalList = NMeshToNList(meshOld[i]);
        var indexList  = IMeshToIList(meshOld[i]);
    }
    viewer.replaceScene(scene);
    viewer.resetScene();
    viewer.update();
